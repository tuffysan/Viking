...
```bash
VERSION=1.7.7 bash -c "$(curl -fsSL https://raw.githubusercontent.com/tuffysan/Viking/main/install-lxc.sh)"
```
...
```bash
CTID=200 CT_HOSTNAME=viking-test STORAGE=local VERSION=1.7.7 \
bash -c "$(curl -fsSL https://raw.githubusercontent.com/tuffysan/Viking/main/install-lxc.sh)"
```
...
Fixar Proxmox-felet dar intern loggtext kunde hamna i `SELECTED_STORAGE`.

Tidigare kunde storage-vardet bli:

```text
Aktiverar rootdir pa storage 'local'...
local
```

Nu gar interna loggrader till stderr och `SELECTED_STORAGE` innehaller endast:

```text
local
```

Installeraren validerar dessutom storage-vardet innan `pct create` kors.
